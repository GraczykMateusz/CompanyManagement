...
class Displayer:
    ...
    def add_employee_button(self, button_image, position_y):
        position_x=220

        self.__employee_button_images_list.append(tk.PhotoImage(file=button_image))
        self.__employee_button=tk.Button(self.root, bd=0, highlightbackground='black', image=self.__employee_button_images_list[self.__employee_buttons_counter])
        self.__employee_button.place(x=position_x, y=position_y)
        self.__employee_buttons_counter+=1

    def add_company_button(self, button_image, position_y):
        position_x=520

        self.__company_button_images_list.append(tk.PhotoImage(file=button_image))
        self.__company_button=tk.Button(self.root, bd=0, highlightbackground='black', image=self.__company_button_images_list[self.__company_buttons_counter])
        self.__company_button.place(x=position_x, y=position_y)
        self.__company_buttons_counter+=1

    def add_database_button(self, button_image, position_y):
        position_x=820

        self.__database_button_images_list.append(tk.PhotoImage(file=button_image))
        self.__database_button=tk.Button(self.root, bd=0, highlightbackground='black', image=self.__database_button_images_list[self.__database_buttons_counter])
